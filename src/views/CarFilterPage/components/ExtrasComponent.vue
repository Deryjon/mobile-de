	...
	export default {
		setup() {
			...
			const isCheckedAmbient = ref(false);
			const isCheckedTyre = ref(false);
			const isCheckedInduction = ref(false);
			const isCheckedSki = ref(false);
			const toggleShowCheckbox = (index) => {
				for (let i = 0; i < isCheckedAlarmSystem.length; i++) {
					if (i !== index) {
						isCheckedAlarmSystem[i] = false;
					}
				}
				isCheckedAlarmSystem[index] = true;
			};

			return {
				isCheckedAlarmSystem,
				isCheckedDisable,
				isCheckedHeated,
				isCheckedSeat,
				isCheckedAmbient,
